...
describe('Given an init service', () => {
  ...
  describe('Given a request to fetch SSM parameters', () => {
    ...
    it('Should return parameters keyed by the last path segment', async () => {
      const paramName = faker.string.alpha(10);
      ...
      expect(result).toEqual({[paramName]: paramValue});
    });

    it('Should return parameters from every page when paginating', async () => {
    ...
    it('Should omit parameters whose Name is undefined', async () => {
      ssmService.getParametersByPath.mockResolvedValueOnce(
        new Mock<GetParametersByPathCommandOutput>()
          .setup(mock => mock.Parameters)
    ...
    it('Should omit parameters whose Value is undefined', async () => {
      ssmService.getParametersByPath.mockResolvedValueOnce(
        new Mock<GetParametersByPathCommandOutput>()
          .setup(mock => mock.Parameters)
          .returns([
            {Name: `/foo/bar/${faker.string.alpha()}`, Value: undefined},
          ])
          .object(),
      );
  ...
  describe('Given a request to eval SSM parameters', () => {
    it('Should write the parameters returned by fetchParameters to process.env', async () => {
      const paramName = `delegated_${faker.string.alpha(10)}`;
      const paramValue = faker.string.alpha();

      const fetchSpy = vi
        .spyOn(service, 'fetchParameters')
        .mockResolvedValueOnce({[paramName]: paramValue});
